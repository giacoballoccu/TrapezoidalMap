...
{
    Algorithms::buildTrapezoidalMap(drawableTrapezoidalMap, dag, segment);
    drawableTrapezoidalMap.setPolygonColors(drawableTrapezoidalMap.getTrapezoidsSize());
}
...
{
    size_t id = Algorithms::queryPoint(drawableTrapezoidalMap, dag, queryPoint, queryPoint);
...
{
    dag.clear();
    dag = Dag(); //Reinizialize in order to have the first node
    drawableTrapezoidalMap.clear();
    drawableTrapezoidalMap = DrawableTrapezoidalMap(); //Reinizialize in order to have the first node
    mainWindow.pushDrawableObject(&drawableTrapezoidalMap, "Trapezoidal Map");
}
